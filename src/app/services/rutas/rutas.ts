...
export class RutasService {
  ...
  createRuta(ruta: Ruta): Observable<Ruta> {
    ...
    return this.http.post<Ruta>(`${this.apiUrl}/rutas?perfil_id=${this.perfilUrl}`, ruta, { headers });
  }
  ...
  updateRuta(id: string, ruta: Ruta): Observable<Ruta> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    return this.http.put<Ruta>(`${this.apiUrl}/rutas/${id}?perfil_id=${this.perfilUrl}`, ruta, { headers });
  }

  deleteRuta(id: string): Observable<void> {
    return this.http.delete<void>(`${this.apiUrl}/rutas/${id}?perfil_id=${this.perfilUrl}`);
  }
}
