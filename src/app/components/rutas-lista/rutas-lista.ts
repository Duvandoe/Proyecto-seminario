...
export class RutasMapaComponent implements AfterViewInit, OnDestroy {
  ...
  successMessage = '';
  errorMessage = '';
  private notificationTimeoutMs = 4000;
 ...
 private cargarRutasDesdeServicio() {
  ...
  this.rutasService.createRuta(payload).subscribe({
    next: (saved: any) => {
      this.saving = false;
      console.log('Respuesta server (createRuta):', saved);
      const added = saved || {};
      ...
      // Si el servidor devuelve un id válido, recargar desde servidor
      // para garantizar consistencia (evita duplicados o formatos inesperados).
      if (added && (added.id || added._id)) {
        // recarga completa: cargarRutasDesdeServicio actualiza `this.rutas` y redibuja
        this.cargarRutasDesdeServicio();
        this.successMessage = 'Ruta guardada correctamente.';
        setTimeout(() => this.successMessage = '', this.notificationTimeoutMs);
      } else {
        // Fallback: si el backend no devuelve id, añadir localmente
        this.rutas.push({
        ...
        this.dibujarRutas();
        this.successMessage = 'Ruta guardada (modo local).';
        setTimeout(() => this.successMessage = '', this.notificationTimeoutMs);
      }
    ...
    error: (err) => {
      ...
      this.errorMessage = 'Error guardando ruta. Revisa la consola.';
      setTimeout(() => this.errorMessage = '', this.notificationTimeoutMs);
    }
  });
}
